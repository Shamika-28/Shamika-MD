...
async function startgss() {
  if(!process.env.SESSION_ID) {
    ...
    console.log(chalk.red("CODED BY SHAMIKA HARSHAMAL"));
...
async function getMessage(key) {
    ...
    return {
        conversation: "Hai im shamika-md botwa",
    };
}
...
gss.ev.on('group-participants.update', async (anu) => {
    ...
    if (connection === 'close') {
        ...
        gss.sendMessage(gss.user.id, {
            text: `*hi bro! 🫡*\n_shamika-md botwa v2 bot has successfully connected to the server_`
        });
    }
});
